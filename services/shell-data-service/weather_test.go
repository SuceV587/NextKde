...
func TestWeatherSubscriptionImmediatelyAnnouncesSnapshot(t *testing.T) {
	service := &Service{weatherSubscribers: map[net.Conn]struct{}{}}
	...
	go func() {
		service.subscribeWeather(server)
		close(done)
	}()
	buffer := make([]byte, len("weather_changed\n"))
	if _, err := client.Read(buffer); err != nil {
		t.Fatal(err)
	}
	if string(buffer) != "weather_changed\n" {
		t.Fatalf("unexpected notification %q", buffer)
	}
	<-done
}
