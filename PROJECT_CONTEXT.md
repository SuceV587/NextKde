...
### P0（路线图最高优先级）
- **数据层收口**: CPU/磁盘/频率采样仍由 QML 承担（`bar/CpuTemperature.qml` 10s 轮询 `/sys`、`/proc`、`df`，写 `usage-history.json` 缓存）。注意内存/温度/活动/在线时长**已在 Go 服务实现**（`main.go` 的 `readMem`/`readTemp`/`settle`），但 UI 仍消费 QML 缓存而非服务快照；Go 服务的 `readCPU` 是 `return 0` 的空壳，`Disk`/`Frequency` 字段从未赋值。`deskcenter/ActivityUsageService.qml` 仍是独立 QML 账本（直接读 `journalctl --list-boots`），需迁入 Go 服务。
- **全局快捷键层**: 缺统一注册与冲突检测。
