...
`apps/` 下的独立应用是桌面 Shell 的可选配套能力，不属于 Shell 的必需运行时。
它们可以与 Shell 同仓维护并共享 UI 或数据契约，但默认的 Shell 使用路径不得
构建、安装或启动这些应用及其服务。每个应用都必须提供独立、可说明的构建与安装
入口，并且只启用自身所需依赖；只使用 Shell 的用户不应被要求安装日历、音乐等
可选应用的依赖。
